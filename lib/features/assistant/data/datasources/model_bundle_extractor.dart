...
/// Extracts one checksum-verified tar.bz2 model archive into staging.
// Extraction remains injectable so repository tests can control lifecycle races.
// ignore: one_member_abstracts
abstract interface class ModelBundleExtractor {
...
/// Streams tar.bz2 model-bundle extraction on a background isolate.
final class IsolateModelBundleExtractor implements ModelBundleExtractor {
  /// Creates the production archive extractor.
  const IsolateModelBundleExtractor();
  ...
  Future<void> extract({
    ...
    return Isolate.run(
      ...
      debugName: 'pov-model-bundle-extraction',
    );
  }
}
  ...
  try {
    ...
    if (actualTarByteSize != expectedTarByteSize) {
      throw ModelIntegrityException(
        reason:
            'the expanded model archive contained $actualTarByteSize '
    ...
    throw ModelIntegrityException(
      reason: 'the pinned model archive could not be extracted: $error',
    );
  } finally {
    if (temporaryTar.existsSync()) temporaryTar.deleteSync();
  }
  ...
  try {
    ...
    if (!decoded) {
      throw const ModelIntegrityException(
        reason: 'the pinned model bzip2 stream is malformed',
      );
    }
  } finally {
    input.closeSync();
  ...
  try {
    ...
    for (final entry in archive) {
      if (entry.isSymbolicLink) {
        throw const ModelIntegrityException(
          reason: 'the pinned model archive contains a symbolic link',
        );
      }
      ...
      if (entry.isDirectory) {
        ...
        throw const ModelIntegrityException(
          reason: 'the pinned model archive contains an unsupported entry',
        );
      }
    }
  } finally {
...
String _safeRelativePath(String archivePath) {
  ...
  if (portablePath.startsWith('/')) {
    throw const ModelIntegrityException(
      reason: 'the pinned model archive contains an absolute path',
    );
  }
  ...
  if (segments.isEmpty || segments.any((segment) => segment == '.' || segment == '..')) {
    throw const ModelIntegrityException(
      reason: 'the pinned model archive contains a path outside staging',
    );
  }
