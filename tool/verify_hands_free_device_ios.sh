...
if [ "$#" -ne 1 ]; then
  echo "usage: tool/verify_hands_free_device_ios.sh <physical-device-id>" >&2
  exit 64
fi
...
flutter drive \
  ...
  --target=integration_test/hands_free_native_test.dart \
  -d "$device_id" \
  --publish-port \
  --dart-define-from-file=.env.example \
  --dart-define=USE_RECORDED_VIDEO=true \
  --dart-define=USE_RECORDED_AUDIO=true \
  --dart-define=RUN_NATIVE_HANDS_FREE_TEST=true
